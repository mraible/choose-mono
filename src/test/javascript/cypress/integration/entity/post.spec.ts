...
describe('Post e2e test', () => {
  ...
  beforeEach(() => {
    ...
    cy.server();
    cy.route('GET', '/api/posts*').as('entitiesRequest');
    cy.visit('');
  ...
  it('should create an instance of Post', () => {
    ...
    cy.route('GET', '/api/posts*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('post');
  ...
  it('should delete last instance of Post', () => {
    cy.server();
    cy.route('GET', '/api/posts*').as('entitiesRequest');
    cy.route('DELETE', '/api/posts/*').as('deleteEntityRequest');
    cy.visit('/');
    cy.clickOnEntityMenuItem('post');
    cy.wait('@entitiesRequest')
      .its('responseBody')
      .then(array => {
        ...
        if (startingEntitiesCount > 0) {
          ...
          cy.route('GET', '/api/posts*').as('entitiesRequestAfterDelete');
          cy.visit('/');
          cy.clickOnEntityMenuItem('post');
