...
describe('Tag e2e test', () => {
  ...
  beforeEach(() => {
    ...
    cy.server();
    cy.route('GET', '/api/tags*').as('entitiesRequest');
    cy.visit('');
  ...
  it('should create an instance of Tag', () => {
    ...
    cy.route('GET', '/api/tags*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('tag');
  ...
  it('should delete last instance of Tag', () => {
    cy.server();
    cy.route('GET', '/api/tags*').as('entitiesRequest');
    cy.route('DELETE', '/api/tags/*').as('deleteEntityRequest');
    cy.visit('/');
    cy.clickOnEntityMenuItem('tag');
    cy.wait('@entitiesRequest')
      .its('responseBody')
      .then(array => {
        ...
        if (startingEntitiesCount > 0) {
          ...
          cy.route('GET', '/api/tags*').as('entitiesRequestAfterDelete');
          cy.visit('/');
          cy.clickOnEntityMenuItem('tag');
