<template>
  <div>
    <h2 id="page-heading" data-cy="BlogHeading">
      <span v-text="$t('blogApp.blog.home.title')" id="blog-heading">Blogs</span>
      <div class="d-flex justify-content-end">
        ...
        >
          ...
          <span v-text="$t('blogApp.blog.home.createLabel')"> Create a new Blog </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && blogs && blogs.length === 0">
      <span v-text="$t('blogApp.blog.home.notFound')">No blogs found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="blogs">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('blogApp.blog.name')">Name</span></th>
            <th scope="row"><span v-text="$t('blogApp.blog.handle')">Handle</span></th>
            <th scope="row"><span v-text="$t('blogApp.blog.user')">User</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="blogApp.blog.delete.question" data-cy="blogDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-blog-heading" v-text="$t('blogApp.blog.delete.question', { id: removeId })">
