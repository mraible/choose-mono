<template>
  <div>
    <h2 id="page-heading" data-cy="PostHeading">
      <span v-text="$t('blogApp.post.home.title')" id="post-heading">Posts</span>
      <div class="d-flex justify-content-end">
        ...
        >
          ...
          <span v-text="$t('blogApp.post.home.createLabel')"> Create a new Post </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && posts && posts.length === 0">
      <span v-text="$t('blogApp.post.home.notFound')">No posts found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('blogApp.post.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('content')">
              <span v-text="$t('blogApp.post.content')">Content</span>
            ...
            <th scope="row" v-on:click="changeOrder('date')">
              <span v-text="$t('blogApp.post.date')">Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('blog.name')">
              <span v-text="$t('blogApp.post.blog')">Blog</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="blogApp.post.delete.question" data-cy="postDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-post-heading" v-text="$t('blogApp.post.delete.question', { id: removeId })">
